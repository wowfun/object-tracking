...
def train(data_dir, net_path=None):
    ...
    for epoch in range(start_epoch, config.epoch + 1):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            ...
            regression_target, conf_target = regression_target.cuda(idx_device), conf_target.cuda(idx_device)
            """---将模板和搜索图像输入net，得到回归参数和分类分数---"""
            #score.shape=[8,10,19,19],regression.shape=[8,20,19,19]
            pred_score, pred_regression = model(exemplar_imgs.cuda(idx_device), instance_imgs.cuda(idx_device))
